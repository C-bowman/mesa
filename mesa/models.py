...
def exponential_transport_profile(x, params):
    ...
    functions = [ lambda x : left_section_exp(x, L_asymp, M_height, L_shape, L_bound),
...
def linear_transport_profile(x, params):
    ...
    L_mid   = params[2]*L_asymp + (1.-params[2])*M_height # left-middle height
    R_mid   = params[3]*R_asymp + (1.-params[3])*M_height # left-middle height
    ...
    functions = [ lambda x : linear_section(x, L_m, L_c),
