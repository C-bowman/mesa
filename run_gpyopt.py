...
while True:
    ...
    # build the domain dictionaries
    domain = [{'name': 'var_'+str(i), 'type': 'continuous', 'domain': bounds} for i,bounds in enumerate(search_bounds)]

    X_step = array(normalised_parameters)
    Y_step = log_posterior.reshape([len(normalised_parameters),1])

    bo_step = BayesianOptimization(
        f=None, domain=domain, X=X_step, Y=Y_step, maximize=True,
        model_type = 'GP', acquisition_type = 'EI', n_samples=6
    )

    new_point = bo_step.suggest_next_locations()
